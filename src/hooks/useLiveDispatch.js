...
export function useLiveDispatch(dispatchId) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const res = await apiClient.get(`/api/admin/dispatches/${dispatchId}`, {
          headers,
        });
  ...
  const kpis = useMemo(() => {
    return {
      ...
      lastLocationAgeSec: latestLocation
        ? (
            (Date.now() - new Date(latestLocation.timestamp).getTime()) /
            1000
          ).toFixed(1)
        : null,
      lastObdAgeSec: latestObd
        ? (
            (Date.now() - new Date(latestObd.timestamp).getTime()) /
            1000
          ).toFixed(1)
        : null,
    };
