...
export const useInsightData = () => {
  ...
  const runningList = useMemo(() => (dispatches || []).filter(d => (d._normStatus || '').toUpperCase() === 'RUNNING'), [dispatches]);
  ...
  useEffect(() => {
    const nextIds = new Set((runningList || []).map(r => r.dispatchId).filter(Boolean));
        ...
        try {
          const off = subscribeDispatchLocation(id, (loc) => {
            setLiveLocations(prev => {
              ...
              let latRaw = loc?.latitude ?? loc?.lat ?? loc?.location?.latitude ?? loc?.position?.lat;
              let lngRaw = loc?.longitude ?? loc?.lng ?? loc?.location?.longitude ?? loc?.position?.lng;
              let lat = (typeof latRaw === 'string') ? parseFloat(latRaw) : latRaw;
              let lng = (typeof lngRaw === 'string') ? parseFloat(lngRaw) : lngRaw;
              if (!isFinite(lat) || !isFinite(lng)) {
                const coords = loc?.coordinates ?? loc?.geometry?.coordinates ?? loc?.point ?? null;
              ...
              if (isFinite(lat) && isFinite(lng)) {
                next.set(id, { lat: Number(lat), lng: Number(lng), ts: Date.now() });
              }
              return next;
            });
          });
          ...
          console.warn('[useInsightData] subscribeDispatchLocation failed for', id, e);
        }
      }
    });
  ...
  const markers = useMemo(() => (runningList || []).map(r => {
    ...
    const lng = live?.lng ?? r.longitude ?? r.lng ?? 126.9780;
  ...
  const loadRunningEventsStats = useCallback(async () => {
    ...
    try {
      ...
      try { console.debug('[useInsightData] loadRunningEventsStats running count:', runningList.length, runningList.slice(0,5).map(x => ({ dispatchId: x.dispatchId, id: x.id, dispatch_id: x.dispatch_id }))); } catch (e) {}
      const requests = runningList.map(r => {
        const id = r.dispatchId ?? r.id ?? r.dispatch_id ?? r.refId ?? r.referenceId;
        if (!id) return Promise.resolve(null);
        return axios.get(`/api/admin/dispatches/${id}/driving-record`, { headers }).then(res => res.data?.data || res.data || null).catch(e => { console.warn('[useInsightData] driving-record failed for', id, e?.message || e); return null; });
      });
      ...
      results.forEach((rec, idx) => {
        ...
        const drowsiness = Number(rec.drowsinessCount ?? rec.drowsiness_count ?? rec.drowsiness ?? 0);
        const acceleration = Number(rec.accelerationCount ?? rec.acceleration_count ?? rec.acceleration ?? 0);
        const braking = Number(rec.brakingCount ?? rec.braking_count ?? rec.braking ?? 0);
        const abnormal = Number(rec.abnormalCount ?? rec.abnormal_count ?? rec.abnormal ?? 0);
        ...
        const score = Number(rec.drivingScore ?? rec.driving_score ?? rec.score ?? rec.driving_score_value ?? NaN);
        if (Number.isFinite(score)) {
          drivingScoreSum += score;
          drivingScoreCount += 1;
          // map to dispatch id if present
          const source = runningList[idx];
          const dispatchId = source?.dispatchId ?? source?.id ?? source?.dispatch_id ?? source?.refId ?? source?.referenceId ?? `idx-${idx}`;
          drivingScorePerDispatch[dispatchId] = score;
        }
      });
      const drivingScoreAverage = drivingScoreCount > 0 ? Math.round((drivingScoreSum / drivingScoreCount) * 10) / 10 : null; // 1 decimal
      try { console.debug('[useInsightData] runningEventsStats computed:', types, { drivingScoreAverage, drivingScorePerDispatch }); } catch (e) {}
      setRunningEventsStats({ ...types, drivingScoreAverage, drivingScorePerDispatch });
    } catch (e) {
  ...
  const subscribedIds = useMemo(() => Array.from(liveLocations.keys()), [liveLocations]);
  ...
  return {
    ...
    // realtime helpers
    liveLocations, subscribedIds,
    // running events aggregation
    runningEventsStats, runningEventsLoading, runningEventsError, loadRunningEventsStats,
  };
};
