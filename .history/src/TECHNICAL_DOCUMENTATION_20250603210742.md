...
**DriverListPanel.jsx - 필터링 및 목록 표시**
```javascript
// 상태별 필터링 로직
...
<div className="overflow-y-auto max-h-96">
  {filteredDrivers.map(driver => (
    <DriverCard key={driver.id} driver={driver} onSelect={onDriverSelect} />
  ))}
</div>
```

**DriverCard.jsx - react-icons을 활용한 상태 표시**
```javascript
// 상태별 아이콘 매핑
const getStatusIcon = (status) => {
  const icons = {
    driving: <IoCarSport className="text-green-500" />,
    waiting: <IoPause className="text-yellow-500" />,
    offline: <IoStop className="text-red-500" />,
    break: <IoCafe className="text-blue-500" />
  };
...
<div className="flex h-screen bg-gray-50">
  ...
  <div className="w-96 bg-white shadow-lg flex flex-col">
    <AlertSummaryWidget />
    <DriverListPanel onDriverSelect={setSelectedDriver} />
  </div>
</div>
