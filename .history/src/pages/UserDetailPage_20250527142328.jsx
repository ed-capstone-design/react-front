...
const UserDetailPage = () => {
  ...
  return (
    <div className="max-w-2xl mx-auto py-10 px-4">
      {/* 프로필 카드 */}
      <div className="bg-gradient-to-r from-blue-100 to-blue-50 rounded-2xl shadow flex items-center gap-6 p-8 mb-8">
        <IoPersonCircle className="text-blue-500 text-7xl drop-shadow" />
        <div>
          <div className="text-2xl font-extrabold text-blue-700 mb-1">{name}</div>
          <div className="text-gray-500 text-sm mb-1">{email}</div>
          <div className="flex gap-2 text-xs">
            <span className="bg-blue-200 text-blue-700 px-2 py-1 rounded">가입일: {joinDate}</span>
            <span className={`px-2 py-1 rounded ${status === "활성" ? "bg-green-200 text-green-700" : "bg-gray-200 text-gray-500"}`}>{status}</span>
          </div>
        </div>
      </div>
      ...
      {/* 정보 수정 폼 */}
      <form
        className="bg-white rounded-2xl shadow p-8 flex flex-col gap-5 mb-8"
        onSubmit={handleSubmit}
      >
        <h3 className="text-xl font-bold text-blue-700 mb-2">기본 정보 수정</h3>
      ...
      <div className="bg-white rounded-2xl shadow p-8">
        <h3 className="text-xl font-bold text-blue-700 mb-4">배차 내역</h3>
        <table className="w-full text-left border-separate border-spacing-y-2">
          ...
          <tbody>
            {dispatchHistory.map((item, idx) => (
              <tr key={idx} className="bg-blue-50 rounded">
                <td className="py-2 px-4 rounded-l">{item.date}</td>
                <td className="py-2 px-4">{item.route}</td>
                <td className="py-2 px-4">{item.time}</td>
                <td className="py-2 px-4 rounded-r">
                  <span className={`px-2 py-1 rounded text-xs font-bold ${item.status === "운행중" ? "bg-green-200 text-green-700" : "bg-gray-200 text-gray-500"}`}>
                    {item.status}
                  </span>
                </td>
              </tr>
