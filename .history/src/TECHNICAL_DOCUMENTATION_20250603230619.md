...
### 5. 사용자 상세 페이지 및 운전 상세 시스템

#### 📁 파일 위치
- `src/pages/UserDetailPage.jsx` - 메인 페이지
- `src/components/UserDetailModal.jsx` - 모달 컴포넌트
- `src/pages/DriveDetail.jsx` - 운전 상세 페이지

#### 🔍 구현 상세

**DriveDetail.jsx - 뒤로가기 기능과 지도 통합**
```javascript
    ...
    <div className="max-w-4xl mx-auto py-10 px-4">
      ...
      <div className="bg-white rounded-xl shadow p-6">
        ...
        <div className="rounded-lg overflow-hidden border border-gray-200">
          <KakaoMap 
            markers={[{
              lat: dummyLocation.lat,
              lng: dummyLocation.lng,
...
// 메인 콘텐츠 렌더링
case "drivedetail":
  return <DriveDetail id={driveDetailId} onBackToInsight={handleBackToInsight} />;
```
...
### 6. 운전자 관리 페이지 (Drivers.jsx)

#### 📁 파일 위치
- `src/pages/Drivers.jsx` - 운전자 관리 메인 페이지

#### 🔍 구현 상세

**Drivers.jsx - 간소화된 인터페이스**
```javascript
const Drivers = ({ onUserNameClick, onEditUserDetail }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto py-10 px-4">
      ...
      {/* UserDetailModal 제거됨 - Home에서 UserDetailPage로 처리 */}
    </div>
  );
};
```
...
3. **이벤트 버블링 제어**: stopPropagation을 활용한 정밀한 상호작용
4. **이중 모달 시스템**: 하나의 컴포넌트에서 여러 모달 동시 관리
5. **콜백 패턴**: 부모-자식 컴포넌트 간 효율적인 통신
6. **타입 안정성**: PropTypes 또는 TypeScript 도입 고려
7. **접근성**: ARIA 속성 및 키보드 네비게이션 지원
8. **반응형 디자인**: 다양한 화면 크기 지원

### 새로운 설계 패턴

**1. 이중 클릭 핸들링**
```javascript
// 카드 전체 클릭과 특정 요소 클릭을 구분
...
// 각 모달은 독립적으로 동작
<UserDetailModal open={userModalOpen} />
<DriverDetailModal open={detailModalOpen} />
```

**3. 페이지 내 네비게이션**
```javascript
// 전체 페이지 이동 없이 메인 컨텐츠만 변경
...
**예시 1: 인사이트에서 운전자 상호작용**
1. 사용자가 운전자 카드 클릭 → UserDetailModal 열림 (메시지 보내기)
2. 사용자가 운전자 이름 클릭 → Home의 handleInsightDriverClick 호출
3. Home에서 setSelected("drivedetail") 실행
4. 메인 컨텐츠가 DriveDetail 컴포넌트로 변경
5. DriveDetail에서 뒤로가기 버튼 클릭 → handleBackToInsight 호출
6. setSelected("insight")로 인사이트 페이지로 복귀

**예시 2: 운전자 관리에서 수정 기능**
1. 사용자가 수정 버튼 클릭 → onEditUserDetail 콜백 호출
2. Home에서 setSelectedUserEmail과 setSelected("userdetail-edit") 실행
3. 메인 컨텐츠가 UserDetailPage로 변경 (편집 모드)
...
### 인사이트 페이지 상호작용 개선
- **이중 클릭 시스템**: 카드 클릭(메시지 보내기) vs 이름 클릭(상세 페이지)
- **DriveDetail 뒤로가기**: 인사이트로 원클릭 복귀 기능
- **지도 통합**: DriveDetail에 실시간 위치 지도 추가

### 운전자 관리 페이지 간소화
- **이름 클릭 기능 제거**: 불필요한 중복 기능 정리
- **수정 버튼 재정의**: UserDetailPage로 직접 연결
- **UI 일관성 개선**: 텍스트 스타일 통일

### 모달 시스템 확장
- **UserDetailModal**: 메시지 보내기 + 위치 정보
- **DriverDetailModal**: 운전자 정보 수정/삭제
- **동시 모달 관리**: 하나의 컴포넌트에서 여러 모달 제어
