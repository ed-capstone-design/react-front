...
const menu = [
  { key: "dashboard", name: "대시보드", icon: <IoSpeedometerOutline /> },
  { key: "drivers", name: "운전자 관리", icon: <IoPeopleOutline /> },
  { key: "operating-schedule", name: "운행 스케줄", icon: <IoCalendarOutline /> },
  { key: "insight", name: "인사이트", icon: <IoBarChartOutline /> },
  { key: "userdetailpage", name: "사용자 상세", icon: <IoPersonCircleOutline /> },
];

const Sidebar = ({ open, onClose, position = "right", selected, onMenuClick }) => {
  const sideClass =
    position === "right"
      ? "right-0 left-auto"
      : "left-0 right-auto";
