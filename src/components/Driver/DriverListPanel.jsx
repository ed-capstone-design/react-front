...
const DriverListPanel = ({ onDriverClick }) => {
    ...
    return(
        <div className="h-[600px] overflow-y-auto pr-1 group relative custom-scrollbar">
            ...
            </div>            {운행중.map((driver) => <div key={driver.id} onClick={() => handleCardClick(driver)} className="cursor-pointer"><DriverCard driver={driver} onNameClick={handleNameClick} /></div>)}
            <hr className="my-2"/>
            {대기.map((driver) => <div key={driver.id} onClick={() => handleCardClick(driver)} className="cursor-pointer"><DriverCard driver={driver} onNameClick={handleNameClick} /></div>)}
            {/* 휴식 상태도 필요시 추가 */}
